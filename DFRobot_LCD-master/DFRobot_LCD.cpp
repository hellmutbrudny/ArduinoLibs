...
/*******************************public*******************************/
DFRobot_LCD::DFRobot_LCD(uint8_t lcd_cols,uint8_t lcd_rows,TwoWire *aWire,uint8_t lcd_Addr,uint8_t RGB_Addr) 
{
  _theWire = aWire;
  _lcdAddr = lcd_Addr;
  _RGBAddr = RGB_Addr;
...
{
    _theWire->beginTransmission(_lcdAddr);        // transmit to device #4
    for(int i=0; i<len; i++) {
        _theWire->write(data[i]);
		delay(5);
    }
    _theWire->endTransmission();                     // stop transmitting
}

void DFRobot_LCD::setReg(uint8_t addr, uint8_t data)
{
    _theWire->beginTransmission(_RGBAddr); // transmit to device #4
    _theWire->write(addr);
    _theWire->write(data);
    _theWire->endTransmission();    // stop transmitting
}
